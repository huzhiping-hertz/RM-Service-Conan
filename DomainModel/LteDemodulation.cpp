...
void LteDemodulation::runTask()
{
	...
	LogInitByDutIndex(DutIndex, logToMode, logLevel, (char*)logPath.c_str(), "LteUlDemodLogFile", true);
	...
	pInputParam->Userseq_hop_enabled = static_cast<SeqHopEnabled>(paramObj["Userseq_hop_enabled"].get<int>());
	pInputParam->Usergroup_hop_enabled = static_cast<GroupHopEnabled>(paramObj["Usergroup_hop_enabled"].get<int>());
	pInputParam->UserBW = paramObj["UserBW"].get<int>();
	pInputParam->UserDMRSBoostdB = paramObj["UserDMRSBoostdB"].get<int>();
	pInputParam->UserModulation = static_cast<LteModulationMode>(paramObj["UserModulation"].get<int>());
